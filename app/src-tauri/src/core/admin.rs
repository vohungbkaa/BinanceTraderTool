...
#[tauri::command]
pub async fn get_top_altcoins_metadata() -> Result<Vec<crate::core::metadata::UniverseCandidate>, String> {
    let rest_client = BinanceRestClient::new();
    let manager = MetadataManager::new(rest_client);
    
    manager.get_universe_candidates().await.map_err(|e| e.to_string())
}
