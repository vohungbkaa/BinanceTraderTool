...
use crate::core::models::{AltcoinSnapshot, MarketIndices, NormalizedCandleData};
use crate::core::rest::BinanceRestClient;
use crate::engine::regime::MarketRegimeEngine;
...
impl SystemSimulator {
    ...
    pub async fn run_historical_validation(&mut self, symbol: &str, limit_4h: u32) -> Result<()> {
        ...
        let mut btc_1d = self
            .rest_client
            .fetch_klines(symbol, "1d", limit_1d)
            .await?;
        btc_1d.reverse();
        let mut btc_4h = self
            .rest_client
            .fetch_klines(symbol, "4h", limit_4h)
            .await?;
        btc_4h.reverse();
        ...
        for alt_sym in &test_alts {
            ...
            let mut a_1d = self
                .rest_client
                .fetch_klines(alt_sym, &alt_tf, limit_1d)
                .await?;
            a_1d.reverse();
            let mut a_4h = self
                .rest_client
                .fetch_klines(alt_sym, "4h", limit_4h)
                .await?;
            a_4h.reverse();
        ...
        for (i, candle_4h) in btc_4h.iter().enumerate() {
            ...
            // Tìm BTC 1D matching
            let matching_btc_1d = btc_processed_1d
                .iter()
            ...
            if let Some((btc_c_1d, btc_i_1d)) = matching_btc_1d {
                ...
                let data_btc_4h = NormalizedCandleData {
                    ...
                    market_indices: MarketIndices {
                        ...
                        btc_d_trend: if candle_4h.close < btc_i_1d.ema200.unwrap_or(0.0) {
                            crate::core::models::TrendDirection::Up
                        } else {
                            crate::core::models::TrendDirection::Down
                        },
                        total3_btc_trend: if candle_4h.close < btc_i_1d.ema200.unwrap_or(0.0) {
                            crate::core::models::TrendDirection::Down
                        } else {
                            crate::core::models::TrendDirection::Up
                        },
                ...
                let context = self
                    .regime_engine
                    .evaluate_historical(&data_btc_1d, &data_btc_4h)
                    .await;
                ...
                if context.allow_alt_scan {
                        ...
                        if let Some((a_hist_1d, a_hist_4h)) = alt_data_map.get(*alt_sym) {
                            ...
                            if let (Some((c4, i4)), Some((c1, _i1))) = (a_4h, a_1d) {
                                current_alt_snapshots.push(AltcoinSnapshot {
                                    ...
                                    distance_to_ema50_4h_pct: (c4.close - i4.ema50.unwrap_or(0.0))
                                        / i4.ema50.unwrap_or(1.0)
                                        * 100.0,
                                    ..Default::default()
                                });
                            }
                    ...
                    let shortlist = self.scanner_engine.scan(
                        &context,
                        btc_change_1d,
                        btc_change_4h,
                        &current_alt_snapshots,
                    );

                    if !shortlist.is_empty() {
                        total_candidates_found += shortlist.len();
                        let date_str = chrono::DateTime::<chrono::Utc>::from_timestamp_millis(
                            candle_4h.close_time / 1000,
                        )
                        ...
                        for alt in shortlist {
                            println!(
                                "   -> 🎯 {} | Rating: {} | RS: {:.2} | Rank: {:.1}",
                                alt.symbol, alt.rs_rating, alt.rs_score, alt.rank_score
                            );
                        }
                    }
                }
        ...
        println!(
            " - Tổng số Altcoin lọt vào Shortlist: {}",
            total_candidates_found
        );
        println!(
            " - Hiệu suất lọc: Trung bình {:.1} coin mỗi khi Gateway mở.",
            total_candidates_found as f64 / trigger_count as f64
        );
        println!("========================================================");
