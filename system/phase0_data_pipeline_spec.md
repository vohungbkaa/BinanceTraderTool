...
Góc nhìn giao dịch Futures chuyên nghiệp: Phase 0 không chỉ trả dữ liệu để "nhìn chart", mà phải phân loại rõ symbol nào **đủ điều kiện thực thi lệnh**. Một altcoin chỉ được đưa vào universe giao dịch khi vượt qua các cổng rủi ro về thanh khoản, độ sâu sổ lệnh, tuổi niêm yết, OI thật và độ lệch funding. Coin không đủ điều kiện vẫn có thể được quan sát, nhưng không được chuyển sang Phase 2 như ứng viên trade.
...
- **Đồng bộ metadata:** Lấy danh sách Symbol hợp lệ, thời điểm niêm yết, funding, OI, OI history, 1D kline và order book định kỳ. Metadata dùng để xây dựng universe tradeable, không chỉ để hiển thị.
...
- **Thanh khoản:** Đo lường độ giãn Spread của Orderbook, độ sâu thực thi cho lệnh chuẩn 50,000 USDT, độ lệch Basis giữa Spot và Futures. Với universe giao dịch futures, spread và slippage là hard gate, không phải chỉ là điểm tham khảo.
...
## 2.4. QUY TẮC XÂY DỰNG UNIVERSE FUTURES TRADEABLE
Phase 0 phải tạo danh sách `universe_candidates` theo nguyên tắc risk-first. Đây là đầu vào cho Phase 2, nên mọi symbol không vượt qua hard gate phải bị loại trước khi tính score.

### A. Hard Gate Bắt Buộc
- **Contract:** Chỉ nhận USDT Perpetual đang `TRADING`.
- **Loại trừ:** BTCUSDT, stablecoin pairs hoặc contract không đại diện cho altcoin tradeable.
- **Listing Age:** >= 30 ngày. Coin mới niêm yết có lịch sử kỹ thuật không đủ tin cậy, spread thường thay đổi nhanh và dễ bị thao túng.
- **Quote Volume 24h:** >= 50M USDT.
- **Open Interest danh nghĩa:** >= 20M USDT.
- **Spread:** <= 0.05%.
- **Estimated Slippage:** Lệnh market giả lập 50,000 USDT không được trượt quá 0.10% ở cả hai phía book. Nếu không lấy được orderbook, symbol không được coi là tradeable.

### B. Metrics Phải Lưu Cho Mỗi Candidate
- `quote_volume`
...
- `taker_buy_ratio_24h`
- `spread_pct`
- `depth_50k_slippage_pct`
- `vol_score`, `oi_score`, `atr_score`, `fund_score`, `liquidity_score`, `flow_score`, `age_score`
- `composite_score`

### C. Composite Score Đề Xuất
Score phải ưu tiên coin có thể vào/thoát lệnh an toàn, không chỉ coin đang chạy mạnh:

`Composite = 18% Volume + 8% VolumeGrowth + 16% OI + 12% OIGrowth + 14% ATR + 8% Funding + 16% Liquidity + 5% TakerFlow + 3% ListingAge`

Diễn giải:
- **Liquidity 16%:** Spread và slippage thấp là điều kiện sống còn khi đánh futures có stoploss ngắn.
- **OI 16% + OI Growth 12%:** Ưu tiên nơi có vị thế thật, tránh volume rỗng.
- **ATR 14%:** Cần đủ biên độ để trade nhưng không quá hỗn loạn.
- **Funding 8%:** Loại crowded trade ở hai đuôi percentile.
- **Taker Flow 5%:** Ratio quá lệch cho thấy dòng market order có thể đang kiệt hoặc bị squeeze; ratio cân bằng/tích cực được ưu tiên hơn.
- **Listing Age 3%:** Sau hard gate 30 ngày, coin lâu đời hơn được điểm ổn định cao hơn nhưng không được áp đảo thanh khoản/dòng tiền.
